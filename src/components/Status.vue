<template>
  <span :title="`Status: ${statusCheck}`">
    <span>
      ...
      <span v-else-if=" integration.definition_id === 'inventory_mgmt'" class="right">
        <button
          :data-cy="`stop-button-${integration.definition_id}`"
          :disabled="disabledAccess"
          v-if="inventoryEnabled"
          class="button danger outline small"
          @click="toggleInventoryEnable(inventoryEnabled)"
        >
...
export default class Status extends Vue {
  ...
  private inventoryEnabled: boolean = true;
  ...
  private async toggleInventoryEnable(invEnabled: boolean): Promise<void> {
    // this.setInventoryEnabledState();
    const inventoryIntegration = this.integration as IntegrationData<InventoryMgmt>;
    inventoryIntegration.metadata.enabled = !this.inventoryEnabled;
    await API.Integration.update(inventoryIntegration);
    this.checkStatus();
  }
  ...
  private setInventoryEnabledState() {
    const inventoryIntegration = this.integration as IntegrationData<InventoryMgmt>;
    this.inventoryEnabled = inventoryIntegration.metadata.enabled;
  }
