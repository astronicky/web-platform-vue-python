...
const linux = {
  async add(integration: IntegrationData<Linux>): Promise<IntegrationCredentials> {
    const organizationId = API.Organization_v2.getCurrentId();
    try {
      const response = await http.post(`linux`, {
  ...
  async remove(integration: Integration<Linux>) {
    const organizationId = API.Organization_v2.getCurrentId();
    // try {
  ...
  async status(integration: Integration<Linux>): Promise<string> {
    const organizationId = API.Organization_v2.getCurrentId();
    // try {
