...
const agent = {
  async add(integration: IntegrationData<Agent>): Promise<IntegrationCredentials> {
    const organizationId = API.Organization_v2.getCurrentId();
    try {
      const response = await http.post(`agent`, {
  ...
  async remove(integration: Integration<Agent>) {
    const organizationId = API.Organization_v2.getCurrentId();
    // try {
  ...
  async status(integration: Integration<Agent>): Promise<string> {
    const organizationId = API.Organization_v2.getCurrentId();
    // try {
